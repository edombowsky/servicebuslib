...
object Retry {
   ...
   * @param backoff           a back-off function that returns a Duration after
   *                          which to retry. Default is an exponential backoff
   *                          at 100 milliseconds steps
   ...
   *
   * @return an eventual Future succeeded with the messages computed or failed with
   *         one of:  `TooManyRetriesException` if there were too many retries
   *         without an exception being caught. Probably impossible if you pass
   *         decent parameters `DeadlineExceededException` if the retry didn't
   *         succeed before the provided deadline `Throwable` the last encountered
   *         exception
   */
  def retry[T](maxRetry: Int,
    ...
    Future[Unit] {
      // Without the _ here will get the message:
      // "eta-expansion of zero-argument method values is depreciated"
      //
      // Advice from: http://tinyurl.com/yc5f6cg5 was followed for a solution.
      recursiveRetry(0, None)(doBlock _)
    }
